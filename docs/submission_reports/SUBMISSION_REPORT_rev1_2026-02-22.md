...
아래 수치는 실제 운영기관 자료가 아니라, 현재 워크플로우를 기준으로 산정한 **사전 추정치**다.  
정식 수치는 파일럿 기간(예: 2~4주) 동안 로그 기반으로 확정한다.

| 구분 | 기존 방식(추정) | ChemIP 활용(추정) | 비고 |
| --- | --- | --- | --- |
| 건당 기본 조회 시간 | 약 120분 | 약 30분 | 안전/특허/시장 분산 조회를 통합 조회로 전환 |
| 반복 입력/복사 정리 | 약 30분 | 약 5분 | 탭 기반 조회와 화면 일원화 가정 |
...
2. MSDS 확인 이후 특허/시장 연계 조회의 단일 흐름화
...
3. 2주 단위로 중간 리포트 작성 후 최종 확정치 반영
